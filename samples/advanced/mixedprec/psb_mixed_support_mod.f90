...
contains
  ...
  subroutine psb_d2s_mvect_col(dv, idx, sv, info, mold)
    ...
    real(psb_spk_), allocatable :: xm(:, :)

    info = psb_success_
    xm = dv%get_vect()
    call sv%bld(xm(:, idx), mold=mold)
  end subroutine psb_d2s_mvect_col
  ...
  subroutine psb_s2d_mvect_col(sv, dv, idx, info, mold)
    ...
    real(psb_dpk_), allocatable :: xm(:, :)

    info = psb_success_
    xm = dv%get_vect()
    xm(:, idx) = sv%get_vect()
    call dv%bld(xm, mold=mold)
  end subroutine psb_s2d_mvect_col
