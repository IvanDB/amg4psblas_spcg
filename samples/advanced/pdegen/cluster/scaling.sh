...
echo "$rows" | sort -k1,1n -k4,4 | awk -F'\t' -v want="$want" -v tolin="${TOL:-1e-6}" '
  BEGIN { tol = tolin + 0 }   # -v hands over a string; force it numeric, or the
                              # comparison is lexicographic and 9.9E-07 comes out
                              # greater than 1e-6
  { nodes=$1; ranks=$2; size=$3; meth=$4; t=$5+0; er=$6+0; it=$7+0
    key=nodes; N[key]=nodes; R[key]=ranks; S[key]=size
    if (meth=="cg") { cg[key]=t; ecg[key]=er; icg[key]=it }
    else            { ss[key]=t; ess[key]=er; iss[key]=it } }
  END {
    ...
    for (i=1;i<=n;i++) {
      ...
      printf "| %5d | %5d | %6s | %10d | %11.5E | %11.5E | %6.3f | %7.2f | %7.2f |%s\n",
             N[k], R[k], S[k], unk, cg[k], ss[k], ss[k]/cg[k], b_cg/cg[k], b_ss/ss[k],
             (ecg[k] > tol ? "  CG DID NOT CONVERGE" : "") \
             (ess[k] > tol ? "  S-STEP DID NOT CONVERGE" : "")
      if (ecg[k] > tol || ess[k] > tol) bad = 1
    }
    if (bad) {
      printf "\nWARNING: rows marked above hit the iteration cap instead of the\n"
      printf "         tolerance. Their timings are cost per iteration of a solve\n"
      printf "         that never finished, and the ss/cg column compares a solved\n"
      printf "         system against an unsolved one. Not a valid comparison.\n"
    }
    printf "\nideal eff: "
